...
class DataIngestion:
    ...
    def unzip_data(self) -> None:
        try:
            logging.info("Initiating Unzipping the downloaded zip file from download directory...")
            raw_zip_path = self.data_ingestion_config.zip_data_path

            if os.path.isdir(self.data_ingestion_config.unzip_data_dir):
                logging.info(f"Data is already available in {self.data_ingestion_config.unzip_data_dir}, Hence skipping unzipping")
            
            else:
                os.makedir(self.data_ingestion_config.unzip_data_path)

                with zipfile.ZipFile(raw_zip_path,"r") as f:
                    f.extractall(self.data_ingestion_config.unzip_data_dir)
                    logging.info(f"Data is unzipped from {raw_zip_path} to {self.data_ingestion_config.unzip_data_dir}")
